...
if( !function_exists( 'accountantlaw_enqueue_styles' ) ) {
	...
	function accountantlaw_enqueue_styles() {
		...
		wp_register_style( 'google_fonts', add_query_arg( $query_args, "//fonts.googleapis.com/css" ), array(), '1.0.7', 'all' );

	    wp_enqueue_style( 'style', get_template_directory_uri() . '/style.css', array(), '1.0.7', 'all' );
	    wp_enqueue_style( 'google_fonts', array(), '1.0.7', 'all' );
	}
}
...
if( !function_exists( 'accountantlaw_enqueue_scripts' ) ) {
	...
	function accountantlaw_enqueue_scripts() {
		wp_enqueue_script( 'scripts', get_stylesheet_directory_uri() . '/js/scripts.js', array(), '1.0.7', true );
	}
}